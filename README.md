...
Русскоязычный проект: документация, интерфейс и подписи на готовой странице — на русском.
...
После установки запускается `/roomwalk:start`. Начать можно двумя способами:

- **Со ссылки на действующий сайт.** Плагин читает его сам — категории, цены, сколько
  фотографий стоит за каждой, фразу, которую бизнес о себе повторяет.
- **С описания словами.** Тогда нужны две вещи: чем занимается бизнес, в одну фразу,
  и его фотографии реальных работ. Без фотографий страница не строится — на остановках
  нечего будет показать.

Дальше приходит план с ценой и **три варианта, как может выглядеть помещение**. Выбор
одного из трёх — единственное решение, которое требуется от человека; всё остальное
считается с сайта.
...
На Windows Python ставится с python.org, и при установке нужна галочка **«Add Python to
PATH»**. Команда там называется `python` или `py`, а не `python3` — плагин это учитывает.
