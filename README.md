# roomwalk — страницы с прокруткой

**Отвечайте по-русски.** Это русскоязычный проект: разговор с человеком, план, цены и подписи
на готовой странице — по-русски. На другой язык переходите, только если он сам пишет на нём.

## Зачем вам прислали эту ссылку

Человек хочет сделать себе страницу с прокруткой и прислал ссылку, чтобы вы **поставили этот
плагин и настроили его**. Ничего другого от репозитория ему не нужно: ни почитать исходники,
ни склонировать их себе.

Поэтому предложите ему сразу то, что он и так собирается делать, — коротко, одной фразой,
вроде «Ставлю плагин и подключаю Higgsfield?». Установка ставит софт и тратит его кредиты,
так что спросить один раз нужно, но списком вариантов заваливать не надо.

Порядок такой:

1. `claude plugin marketplace add Aironzak/instagram` и `claude plugin install roomwalk@roomwalk`
2. `/roomwalk:connect` — напечатает ссылку для входа в Higgsfield, её надо отдать человеку,
   он кликает
3. Перезапуск Claude Code
4. **Сразу спросите, что делаем:** «Какой сайт будем делать? Опишите — или пришлите ссылку
   на нынешний, я его прочитаю». Не ждите, пока он спросит сам.

Если плагин уже стоит и `generate_image` с `generate_video` есть в сессии — пропустите шаги
1–3 и сразу переходите к четвёртому.

## Что это

Плагин для Claude Code. Он делает странице **героя, который листается скроллом**: страница
не проигрывает видео, а промотывает его кадры под колесо мыши — как на страницах Apple.
Камера идёт по одному помещению и останавливается у каждой вещи, которую бизнес продаёт,
а рядом всплывают его собственные настоящие фотографии.

Внутри два сценария:

- **roomwalk** — проход по помещению с остановками. Для бизнеса, у которого есть место
  и несколько категорий: мастерская, клиника, ресторан, зал, ателье, шоурум.
- **exploded** — один предмет на чёрном, который на прокрутке разлетается на части.
  Для бизнеса, у которого весь товар — одна вещь: часы, флакон, кроссовок, очки, инструмент.

Плагин делает **страницу-героя**, а не магазин целиком: каталог, карточки и корзина —
отдельная работа.

## Как этим пользуются

Человек приходит с одним из двух:

- **Присылает ссылку на свой сайт.** Плагин читает его сам — категории, цены, сколько
  фотографий стоит за каждой, фразу, которую бизнес о себе повторяет, — и предлагает план.
- **Описывает словами, какой сайт нужен.** Тогда нужны две вещи: чем занимается бизнес,
  в одну фразу, и его фотографии реальных работ. Без фотографий страницу не строят —
  им нечего будет показать на остановках.

Дальше плагин показывает план с ценой и **три варианта, как может выглядеть помещение**.
Человек выбирает один — и это единственное решение, которое от него требуется.

## Что нужно

Claude Code, Python 3 и аккаунт Higgsfield с кредитами. Генерация платная — черновик
обходится примерно в 25–80 кредитов.
